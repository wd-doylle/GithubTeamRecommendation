...
class GATDataset():
    ...
    def __init__(self,config):
        ...
        # self.repo_features = self.repo_features[:500]
    ...
    def get_data_loader(self, device, stages):
        ...
        for stage in stages:
            ...
            for rp in data_range:
                ...
                repo_users = [user_inds[u] for u in self.repo_core_users[rp]]
                users = torch.tensor([self.user_features[u] for u in self.repo_users[rp]],device=device)
                e1 = []
                e2 = []
                for u1 in self.repo_core_users[rp]:
                    e1.append(user_inds[u1])
                    e2.append(user_inds[u1])
                    for u2 in self.user_social[u1]:
                        e1.append(user_inds[u1])
                        e2.append(user_inds[u2])
                        if not u2 in self.repo_core_users[rp]:
                            e2.append(user_inds[u1])
                            e1.append(user_inds[u2])
                user_edges = torch.tensor([e1,e2],dtype=torch.long,device=device)
                ...
                target = torch.tensor([0]*nm_zeros+[1]*nm_ones,dtype=torch.float,device=device).view(-1,1)
                data[-1].append([repo, repo_users,users,user_edges,teams,team_users,target])

                self.repo_features.pop(rp)
                self.repo_core_users.pop(rp)
                self.repo_users.pop(rp)
